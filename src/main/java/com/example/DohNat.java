...
public class DohNat {
  ...
  /** This constructor creates a planet divided into a grid of
   ...
   jolts:   number of jolts that a plant harvest delivers
   growth:  number of rounds it takes to grow plant plants
   */
  public DohNat( int height, int width, int jolts, int growth ) {
    ...
    for( int x = 0; x < width; x++ ) {
      for( int y = 0; y < height; y++ ) {
        districts[x][y] = new District(this, "spresso", x, y, jolts, growth);
      }
    }
  }
  ...
  /** This method executes the Fire phase of the round.
   */
  ...
  /**
   ...
   the grid.   There should be no spaces separating the districts, and
   each row of districts should be terminated by a newline.
   */
